...
[
  ...
  if (test "$HAVE_MD5SUM" != "no"); then
    ...
    if (test "$HAVE_SHASUM" != "no"); then
      ...
      AC_MSG_ERROR(No file/directory hash computation program is available. Report to moab-dev@mcs.anl.gov)
    fi
  fi
  ...
  if (test "x$MKDIR_P" != "x"); then
    ...
    AC_MSG_ERROR(Make directory command not found ? Seriously ? Report to moab-dev@mcs.anl.gov)
  fi
  AC_PROG_MKDIR_P
  AC_PROG_MAKE_SET
...
[
      ...
      if (test "$HAVE_WGET" != "no" ); then
        ...
        if (test -f "$3"); then
          ...
          if (test "$op_checkifexists" != "yes"); then
            AC_MSG_ERROR( --  Requested URL does not exist in remote host. Try again later. ($2)  -- )
          fi
  ...
  if (test "$filedownloaded" != "yes"); then
    AC_MSG_ERROR( --  The archive URL ($2) specified cannot be handled by wget, curl or scp  -- )
  else
    if (test "$verbosemessages" != "no" ); then
...
[
  ...
  if (test "$filedownloaded" != "yes"); then
    AC_MSG_ERROR( --  The Git repository URL ($2) specified could not be cloned  -- )
  else
     pkg_srcdir="$4"
     MSG_ECHO_LOG(${op_downloadlog$1})
  fi
...
[
  ...
  if (test "x`basename $2|grep -E '\.tar'`" != "x" && test "$HAVE_TAR" != "no" ); then
    ...
    AC_MSG_ERROR( --  Unhandled file format for deflating package $1 -- Filename = $2 -- )
  fi
  ...
  if (test "x$op_pkg_subdir" != "x"); then
    ...
    AC_MSG_ERROR( --  Unhandled file format for getting the source tree name for package $1 -- Filename = $2 -- )
  fi
])
