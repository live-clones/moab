/**
 * imoab_ola_base.cpp: will do only trivial partitioning, no zoltan
 *
...
int main( int argc, char* argv[] )
{
    ...
    if( couComm != MPI_COMM_NULL )
    {
        char outputFileTgt3[] = "recvOcn6.h5m";
    ...
    if( couComm != MPI_COMM_NULL )
    {
        char outputFileAtmInf[] = "recvAtm6.h5m";
        PUSH_TIMER( couComm, "Write migrated ATM mesh on coupler PEs, inferred from OCN" )
    ...
    {  // write only for n==1 case
        char outputFileLnd[] = "recvLnd6.h5m";
    ...
    if( lndCouComm != MPI_COMM_NULL )
    {
        ...
        if( lndComm != MPI_COMM_NULL )
        {
           ...
           int context   = cplatm;
           int verbo = 1;
           iMOAB_DumpCommGraph( cmpLndPID, &context, &is_sender, &verbo, "LndSndAtmC" );
        }
        if( couComm != MPI_COMM_NULL )
        {
           int is_sender = 0;
           int context   = cplatm;
           int verbo = 1;
           iMOAB_DumpCommGraph( cplLndPID, &context, &is_sender, &verbo, "LndRecAtmC" );
        }
#endif
    }
    ...
    if( couComm != MPI_COMM_NULL )
    {
        ...
        ierr = iMOAB_WriteMesh( cplOcnAtmPID, (char*)intxfile.c_str(), serialWriteOptions );
    ...
    if( couComm != MPI_COMM_NULL )
    {
        ...
        ierr = iMOAB_ComputeScalarProjectionWeights(
            ...
            &fNoBubble, &fMonotoneTypeID, &fVolumetric, &fInverseDistanceMap, &fNoConserve, &fValidate,
    ...
    if( couComm != MPI_COMM_NULL )
    {
        ...
        if (!skip_writing_map)
        {
            ...
            std::string lnd_atm_map_file_name = "lnd_atm_map_test_ola.nc";
    ...
    const char* bottomTempField          = "T_proj:u_proj:v_proj";  // on ocean and land input files
    const char* bottomTempProjectedField = "T_proj2:u_proj2:v_proj2";

    // coming from lnd to atm;  ( PS: will have to merge projection from ocn and land on atm )
    const char* bottomTempProjectedField3 = "T_proj3:u_proj3:v_proj3";
    ...
    // make the tag 0, to check we are actually sending needed data
    {
        if( cplOcnAppID >= 0 )
        {
            ...
            int storLeng = ocnCompNDoFs * numAllElem * 3; // we have 3 tags now
    ...
    if( atmComm != MPI_COMM_NULL )
    {
        ...
        CHECKIERR( ierr, "failed to define the field tag for receiving back the tag *2 on atm pes" );
    }
    ...
    if (analytic_field && (ocnComm != MPI_COMM_NULL) ) // we are on ocean pes
    {
        ...
        int numAllElem = nelem[2];
        std::vector< double > vals;
        int storLeng = ocnCompNDoFs * numAllElem * 3;
        vals.resize( storLeng );
        for( int k = 0; k < storLeng; k++ )
            vals[k] = k;
        int eetype = 1;
        ierr       = iMOAB_SetDoubleTagStorage( cmpOcnPID, bottomTempField, &storLeng, &eetype, &vals[0] );
        CHECKIERR( ierr, "cannot make tag *_proj null" )
    ...
    for( int iters = 0; iters < n; iters++ )
    {
        ...
        if( ocnComm != MPI_COMM_NULL )
        {
            ...
            ierr = iMOAB_SendElementTag( cmpOcnPID, bottomTempField, &ocnCouComm, &cplatm );
            CHECKIERR( ierr, "cannot send tag values" )
        }
        if( couComm != MPI_COMM_NULL )
        {
            // receive on ocn on coupler pes, that was redistributed according to coverage
            ierr = iMOAB_ReceiveElementTag( cplOcnPID, bottomTempField, &ocnCouComm, &cplatm );
        ...
        */
        ...
        if( couComm != MPI_COMM_NULL )
        {
            ...
            ierr = iMOAB_ApplyScalarProjectionWeights( cplOcnAtmPID, weights_identifiers[0], bottomTempField,
        ...
        PUSH_TIMER( MPI_COMM_WORLD, "Send data from cpl atm back to atm pes" )
        if( couComm != MPI_COMM_NULL )
        {
            context_id = cmpatm;
            ierr = iMOAB_SendElementTag( cplAtmPID, bottomTempProjectedField, &atmCouComm, &context_id );
        ...
        {
            context_id = cplatm;
            ierr = iMOAB_ReceiveElementTag( cmpAtmPID, bottomTempProjectedField, &atmCouComm, &context_id );
        ...
        if( lndComm != MPI_COMM_NULL )
        {
            // as always, use nonblocking sends
            // this is for projection to atm, from ocean:
            ierr = iMOAB_SendElementTag( cmpLndPID, bottomTempField, &lndCouComm, &cplatm );
            CHECKIERR( ierr, "cannot send tag values" )
        }
        if( couComm != MPI_COMM_NULL )
        {
            // receive on lnd on coupler pes, that was redistributed according to coverage
            ierr = iMOAB_ReceiveElementTag( cplLndPID, bottomTempField, &lndCouComm, &cplatm );
            CHECKIERR( ierr, "cannot receive tag values" )
        }
        ...
        if( couComm != MPI_COMM_NULL )
        {
            /* We have the remapping weights now. Let us apply the weights onto the tag we defined
               on the source mesh and get the projection on the target mesh */
            PUSH_TIMER( couComm, "Apply Scalar projection weights" )
            ierr = iMOAB_ApplyScalarProjectionWeights( cplLndAtmPID, weights_identifiers[1], bottomTempField,
            		bottomTempProjectedField3 );
            CHECKIERR( ierr, "failed to compute projection weight application" );
            POP_TIMER( couComm, rankInCouComm )
            if( 1 == n )
            {
        ...
        PUSH_TIMER( MPI_COMM_WORLD, "Send/receive data from atm cpl to atm comp/land proj" )
        if( couComm != MPI_COMM_NULL )
        {
            context_id = cmpatm;
            ierr = iMOAB_SendElementTag( cplAtmPID, bottomTempProjectedField3, &atmCouComm, &context_id );
            CHECKIERR( ierr, "cannot send tag values back to atm pes " )
        }
        ...
        {
            context_id = cplatm;
            ierr = iMOAB_ReceiveElementTag( cmpAtmPID, bottomTempProjectedField3, &atmCouComm, &context_id );
            CHECKIERR( ierr, "cannot receive tag values from atm mesh on coupler pes" )
        }
